...
export default function EnemyUnit({ id, initialPosition, health, maxHealth, onTakeDamage, playerPosition, onAttackPlayer }: EnemyUnitProps) {
  ...
  useEffect(() => {
    ...
    if (health === 0) {
      ...
      regenerationTimeoutRef.current = setTimeout(() => {
        ...
        onTakeDamage(id, maxHealth); // Restore to max health
      }, 5000);
    }
  ...
  }, [health, onTakeDamage, id, maxHealth]);
  ...
  return (
    <group 
      ref={enemyRef} 
      visible={true} // Always keep visible to prevent blinking
    >
      <CustomSkeleton 
        position={[0, 0, 0]}
