...
const FrostTrail: React.FC<FrostTrailProps> = ({
  color,
  size,
  meshRef,
  opacity = 1
  ...
  // ref to store the last known position for smoother updates (like FireballTrail)
  ...
  useFrame(() => {
    ...
    // Only update if position has changed significantly (like FireballTrail)
    if (worldPosition.distanceToSquared(lastKnownPosition.current) > 0.0001) {
      lastKnownPosition.current.copy(worldPosition);

      // Update particle positions by shifting them backward (FireballTrail approach)
    ...
    // Update opacities and scales with parent opacity (FireballTrail style)
    for (let i = 0; i < particlesCount; i++) {
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            // Simpler frost effect similar to FireballTrail but with cooler tones
            vec3 glowColor = mix(uColor, vec3(0.9, 1.0, 1.0), 0.3);
