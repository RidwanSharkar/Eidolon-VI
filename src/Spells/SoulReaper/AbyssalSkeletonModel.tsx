...
function AbyssalSkeletonLegModel() {
...
function AbyssalSkeletonClawModel() {
  const createBoneSegment = (length: number, width: number) => (
    <mesh geometry={largeBoneGeometry} material={standardBoneMaterial} scale={[width/0.044, length/1.1, width/0.044]} />
  );
...
function AbyssalSkeletonShoulderPlate() {
  return (
    <group>
      <group>
        ...
        {/* Elaborate shoulder spikes - malachite green style */}
        {[0, 1, 2].map((i) => {
        ...
        {[-1, 1].map((side) => (
          <group key={`blade-${side}`}>
            ...
            <BladeDecoration 
              ...
              rotation={[Math.PI/4, side * Math.PI/8, side * Math.PI/16]}
            />
            ...
            <BladeDecoration 
              ...
              rotation={[Math.PI/6, side * Math.PI/4, side * Math.PI/8]}
            />
            ...
            <BladeDecoration 
              ...
              rotation={[0, side * Math.PI/3, side * Math.PI/4]}
            />
          </group>
        ))}
      </group>
...
export default function AbyssalSkeletonModel({ 
  position, 
  isAttacking, 
  isWalking, 
  onHit,
  ...
  const walkSpeed = 2.5; // Similar to regular skeleton but slightly slower
  const attackSpeed = 2.75; // Faster attack animation for quicker downswing
    ...
    if (isUsingFrostStrike) {
      ...
      if (leftArm && rightArm) {
        if (progress < Math.PI/3) {
          ...
          // Add green glow effect to hands during casting
  ...
  return (
    ...
    >
      ...
      {/* Head - death knight skull with green eyes */}
      <group name="Head" position={[0, 1.54, 0.44]} scale={[0.825, 0.825, 0.825]} rotation={[0.15, 0, 0]}>
        ...
        <group>
          ...
          <group position={[0, 0.1, 0]}>
            ...
            {/* Decorative blade patterns on helmet */}
            {/* Side blade decorations - much larger and more prominent */}
            {[-1, 1].map((side) => (
              <group key={`helmet-blade-${side}`}>
                {/* Main side blade - dramatically larger */}
                <BladeDecoration 
                  scale={0.7}
                  position={[side * 0.25, 0.1, 0.08]}
                  rotation={[Math.PI/8, side * Math.PI/4, side * Math.PI/6]}
                />
                
                {/* Upper side accent blade - larger */}
                <BladeDecoration 
                  scale={0.5}
                  position={[side * 0.2, 0.15, -0.02]}
                  rotation={[Math.PI/6, side * Math.PI/6, side * Math.PI/8]}
                />
                
                {/* Lower side blade for more coverage */}
                <BladeDecoration 
                  scale={0.4}
                  position={[side * 0.28, 0.02, 0.12]}
                  rotation={[0, side * Math.PI/3, side * Math.PI/4]}
                />
              </group>
            ))}
      ...
      {/* Shoulder plates - malachite green armor */}
      <group position={[-0.385, 1.496, 0.22]} rotation={[0.2, -Math.PI - 0.1, -0.1]}>
        <AbyssalSkeletonShoulderPlate />
      </group>
      <group position={[0.385, 1.496, 0.22]} rotation={[0.2, Math.PI - 0.1, 0.1]}>
        <AbyssalSkeletonShoulderPlate />
      </group>
