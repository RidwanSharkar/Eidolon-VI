...
const ABYSSAL_HORN_SEGMENTS = 10;
const ABYSSAL_HORN_HEIGHT_PER_SEGMENT = 0.12;
const ABYSSAL_HORN_BASE_WIDTH = 0.15;

// Pre-compute cylinder geometries for each segment (avoid dynamic args in JSX)
const ABYSSAL_HORN_CYLINDER_GEOMETRIES = Array.from({ length: ABYSSAL_HORN_SEGMENTS }).map((_, i) => {
  const progress = i / (ABYSSAL_HORN_SEGMENTS - 1);
  const width = ABYSSAL_HORN_BASE_WIDTH * (1 - progress * 0.8);
  return new CylinderGeometry(width, width * 0.92, ABYSSAL_HORN_HEIGHT_PER_SEGMENT, 4);
});

// Pre-compute box geometries for ridge details (4 per segment)
const ABYSSAL_HORN_RIDGE_GEOMETRIES = Array.from({ length: ABYSSAL_HORN_SEGMENTS }).map((_, i) => {
  const progress = i / (ABYSSAL_HORN_SEGMENTS - 1);
  const width = ABYSSAL_HORN_BASE_WIDTH * (1 - progress * 0.8);
  return new BoxGeometry(width * 0.4, ABYSSAL_HORN_HEIGHT_PER_SEGMENT * 1.1, width * 0.2);
});

// Pre-compute materials for each segment (avoids dynamic color string in JSX)
const ABYSSAL_HORN_CYLINDER_MATERIALS = Array.from({ length: ABYSSAL_HORN_SEGMENTS }).map((_, i) => {
  const progress = i / (ABYSSAL_HORN_SEGMENTS - 1);
...
const ABYSSAL_HORN_RIDGE_MATERIALS = Array.from({ length: ABYSSAL_HORN_SEGMENTS }).map((_, i) => {
  const progress = i / (ABYSSAL_HORN_SEGMENTS - 1);
  return new MeshStandardMaterial({
...
let registeredAbyssalResources = false;
const registerAbyssalResources = () => {
  if (registeredAbyssalResources || typeof window === 'undefined') return;
  try {
    registerGlobalSharedResource(() => {
      ...
      ABYSSAL_HORN_CYLINDER_GEOMETRIES.forEach(geo => geo.dispose());
      ABYSSAL_HORN_RIDGE_GEOMETRIES.forEach(geo => geo.dispose());
      ABYSSAL_HORN_CYLINDER_MATERIALS.forEach(mat => mat.dispose());
      ABYSSAL_HORN_RIDGE_MATERIALS.forEach(mat => mat.dispose());
    }, 'AbyssalAbomination');
    registeredAbyssalResources = true;
  } catch (error) {
    console.warn('Failed to register Abyssal resources:', error);
  }
};
...
function EyeSet({ position }: { position: [number, number, number] }) {
  ...
  return (
    <group>
      ...
      <pointLight color="#00FF00" intensity={0.5} distance={1} decay={2} position={position} />
    </group>
  );
}

interface AbyssalAbominationProps {
          ...
          <group rotation={[-0.7, -0, Math.PI / 5]}>
            ...
            <group position={[0, -0.5, 0]} rotation={[0, 0, Math.PI / 5.5]}>
              ...
              <group position={[0, -0.1, 0]}>
                ...
                <mesh geometry={SHARED_GEOMETRIES.handBase} material={SHARED_MATERIALS.standardBone} />
                {[-0.08, -0.04, 0, 0.04, 0.08].map((offset, i) => (
                  ...
                  >
                    ...
                    <group position={[0.025, -0.3, 0]} rotation={[0, 0, Math.PI + Math.PI / 16]}>
                      <mesh geometry={SHARED_GEOMETRIES.clawCone} material={SHARED_MATERIALS.darkBone} />
                    </group>
                  </group>
                ))}
              </group>
...
function CustomHorn({ isLeft = false }: { isLeft?: boolean }) {
  ...
  return (
    <group rotation={[-0.25, isLeft ? -0.3 : 0.3, isLeft ? -0.4 : 0.4]}>
      {Array.from({ length: ABYSSAL_HORN_SEGMENTS }).map((_, i) => {
        const progress = i / (ABYSSAL_HORN_SEGMENTS - 1);
        const width = ABYSSAL_HORN_BASE_WIDTH * (1 - progress * 0.8);
        ...
        return (
          <group 
            ...
            position={[
              ...
              i * ABYSSAL_HORN_HEIGHT_PER_SEGMENT,
              -curve * 0.6
            ]}
          ...
          >
            ...
            <mesh
              geometry={ABYSSAL_HORN_CYLINDER_GEOMETRIES[i]}
              material={ABYSSAL_HORN_CYLINDER_MATERIALS[i]}
            />
            ...
            {Array.from({ length: 4 }).map((_, j) => (
              ...
              >
                <mesh 
                  ...
                  geometry={ABYSSAL_HORN_RIDGE_GEOMETRIES[i]}
                  material={ABYSSAL_HORN_RIDGE_MATERIALS[i]}
                />
              </group>
            ))}
          </group>
...
export default function AbyssalAbomination({ position, isAttacking, isWalking }: AbyssalAbominationProps) {
  // Register shared resources on first use
  useEffect(() => {
    registerAbyssalResources();
  }, []);
  ...
  return (
    <group ref={groupRef} position={[position[0], position[1], position[2]]} scale={[0.88, 1.04, 1.04]}> {/* 0.8 scale */}
      ...
      <group name="Head" position={[0, 1.925, 0.2]} scale={[ 0.95, 0.8, 0.8]}>
        ...
        {/* EYES============================= */}
        {/* Eye sockets with glow effect - GREEN THEMED */}
