...
const PyroclastTrail: React.FC<PyroclastTrailProps> = ({
  color,
  size,
  meshRef,
  opacity = 1
}) => {
  const particlesCount = 34; 
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < particlesCount; i++) {
      ...
      scalesRef.current[i] = size * 0.6 * Math.pow((1 - i / particlesCount), 0.5);
    }
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            // Intense fire red effect with bright core
            vec3 glowColor = mix(uColor, vec3(1.0, 0.3, 0.1), 0.6);
