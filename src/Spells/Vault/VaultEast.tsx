...
interface VaultEastProps {
  parentRef: React.RefObject<Group>;
  isActive: boolean;
  onComplete: () => void;
}

const VAULT_DISTANCE = 3.5; // Distance in units to vault to the right
const VAULT_DURATION = 0.25; // Duration in seconds
const MAX_VAULT_BOUNDS = 25; // Maximum distance from origin

export default function VaultEast({ parentRef, isActive, onComplete }: VaultEastProps) {
  ...
  useFrame(() => {
    ...
    const newPosition = startPosition.current.clone().add(
      rightDirection.multiplyScalar(VAULT_DISTANCE * easeOutQuad)
    );
