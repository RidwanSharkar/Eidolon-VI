...
interface SummonedMageProps {
    position: [number, number, number];
    isAttacking: boolean;
    isWalking: boolean;
    onHit: (targetId: string, damage: number, position: Vector3) => void;
}
...
function StaffModel({ isLeftHand = false }: { isLeftHand?: boolean }) {
  return (
    <group rotation={[0, 0, isLeftHand ? Math.PI / 3 : -Math.PI / 3]}>
      {/* Staff handle */}
      <mesh>
...
function MageRobe() {
  return (
    <group>
      {/* Main robe body */}
      <mesh position={[0, 0, 0]}>
        <cylinderGeometry args={[0.4, 0.8, 1.6, 8]} />
        <meshStandardMaterial 
          color="#2a0845"
          roughness={0.7}
          transparent
      ...
      {/* Robe trim */}
      <mesh position={[0, 0, 0]}>
        <cylinderGeometry args={[0.81, 0.82, 0.1, 8]} />
        <meshStandardMaterial 
          color="#6b0fb3"
          metalness={0.3}
          roughness={0.6}
      ...
      {[-1, 1].map((side) => (
        <group 
          key={side}
          position={[0.3 * side, 1.4, 0]}
          rotation={[0.2, 0, 0.3 * side]}
        >
          <mesh>
...
export default function SummonedMage({ position, isAttacking, isWalking, }: SummonedMageProps) {
