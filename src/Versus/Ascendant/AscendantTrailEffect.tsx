...
const AscendantTrailEffect: React.FC<AscendantTrailEffectProps> = ({ parentRef }) => {
  ...
  useFrame((state, delta) => {
    ...
    const ascendantPosition = parentRef.current.position;
    ...
    for (let i = 0; i < particlesCount; i++) {
      ...
      positionsRef.current[i * 3] = ascendantPosition.x + Math.cos(angle) * radius;
      positionsRef.current[i * 3 + 1] = ascendantPosition.y + Math.sin(timeRef.current + i * 0.1) * 0.0001;
      positionsRef.current[i * 3 + 2] = ascendantPosition.z + Math.sin(angle) * radius;
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            vec3 glowColor = mix(vec3(0.8, 0.0, 0.0), vec3(1.0, 0.2, 0.2), 0.4);
...
export default AscendantTrailEffect; 