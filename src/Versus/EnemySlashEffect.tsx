import { useRef, useMemo, useEffect } from 'react';
import { Group, Vector3 } from 'three';
import { useFrame } from '@react-three/fiber';
import * as THREE from 'three';
import { geometryPools, materialPools } from '@/Scene/EffectPools';

interface EnemySlashEffectProps {
  startPosition: Vector3;
  direction: Vector3;
  onComplete: () => void;
}

export default function EnemySlashEffect({ 
  startPosition, 
  direction, 
  onComplete
}: EnemySlashEffectProps) {
  const effectRef = useRef<Group>(null);
  const progressRef = useRef(0);
  const animationDuration = 0.4; // Match with attack timing

  // Use pooled geometries and materials
  const pooledResources = useMemo(() => {
    const geometries = {
      mainSlash: geometryPools.slashMainTorus.acquire(),
      innerGlow: geometryPools.slashInnerGlow.acquire(),
      outerGlow: geometryPools.slashOuterGlow.acquire(),
      particle: geometryPools.slashParticle.acquire(),
      trailSegment: geometryPools.slashTrailSegment.acquire()
    };

    const materials = {
      mainSlash: materialPools.slashMain.acquire(),
      innerGlow: materialPools.slashInnerGlow.acquire(),
      outerGlow: materialPools.slashOuterGlow.acquire(),
      particle: materialPools.slashParticle.acquire(),
      trail: materialPools.slashTrail.acquire()
    };

    return { geometries, materials };
  }, []);

  // Return resources to pool on cleanup
  useEffect(() => {
    return () => {
      const { geometries, materials } = pooledResources;
      
      // Return geometries to pool
      geometryPools.slashMainTorus.release(geometries.mainSlash);
      geometryPools.slashInnerGlow.release(geometries.innerGlow);
      geometryPools.slashOuterGlow.release(geometries.outerGlow);
      geometryPools.slashParticle.release(geometries.particle);
      geometryPools.slashTrailSegment.release(geometries.trailSegment);
      
      // Return materials to pool
      materialPools.slashMain.release(materials.mainSlash);
      materialPools.slashInnerGlow.release(materials.innerGlow);
      materialPools.slashOuterGlow.release(materials.outerGlow);
      materialPools.slashParticle.release(materials.particle);
      materialPools.slashTrail.release(materials.trail);
    };
  }, [pooledResources]);

  // Create particle positions along the slash arc
  const particlePositions = useMemo(() => 
    Array(8).fill(0).map((_, i) => {
      const angle = (i / 7) * Math.PI * 0.8 - Math.PI * 0.4; // Arc from -40% to 40% of PI
      return {
        position: new THREE.Vector3(
          Math.cos(angle) * 1.2,
          Math.sin(angle) * 1.2,
          0
        ),
        delay: i * 0.05 // Stagger particle appearance
      };
    }), 
  []);

  // Trail positions for sword trail effect
  const trailPositions = useMemo(() => 
    Array(6).fill(0).map((_, i) => {
      const angle = (i / 5) * Math.PI * 0.8 - Math.PI * 0.4;
      return {
        position: new THREE.Vector3(
          Math.cos(angle) * 0.8,
          Math.sin(angle) * 0.8,
          -0.1
        ),
        delay: i * 0.03
      };
    }), 
  []);

  useFrame((_, delta) => {
    if (!effectRef.current) return;

    progressRef.current += delta;
    const progress = Math.min(progressRef.current / animationDuration, 1);

    // Scale and fade animation - keep effect at fixed position
    const scale = Math.sin(progress * Math.PI) * 1.2; // Peak at middle, fade at ends
    const opacity = Math.sin(progress * Math.PI); // Same fade pattern
    
    if (effectRef.current) {
      effectRef.current.scale.set(scale, scale, scale);
      
      // Update material opacities using pooled materials
      const { materials } = pooledResources;
      if (materials.mainSlash) materials.mainSlash.opacity = opacity * 0.9;
      if (materials.innerGlow) materials.innerGlow.opacity = opacity * 0.8;
      if (materials.outerGlow) materials.outerGlow.opacity = opacity * 0.6;
      if (materials.particle) materials.particle.opacity = opacity * 0.7;
      if (materials.trail) materials.trail.opacity = opacity * 0.6;
    }

    // Complete animation
    if (progress >= 1) {
      onComplete();
    }
  });

  return (
    <group
      ref={effectRef}
      position={startPosition.toArray()}
      rotation={[
        0, // Keep level with ground
        Math.atan2(direction.x, direction.z) + Math.PI/2, // Face direction
        Math.PI/2 // Rotate 90 degrees to make vertical (top to bottom)
      ]}

      /*  PARALLEL TO GROUND     rotation={[
        Math.PI/2, // Make perpendicular to ground (90 degrees)
        Math.atan2(direction.x, direction.z), // Face direction
        0 // Remove tilt for cleaner vertical slash
      ]} */ 
    >
      {/* Main slash arc */}
      <mesh geometry={pooledResources.geometries.mainSlash} material={pooledResources.materials.mainSlash} />
      <mesh geometry={pooledResources.geometries.innerGlow} material={pooledResources.materials.innerGlow} />
      <mesh geometry={pooledResources.geometries.outerGlow} material={pooledResources.materials.outerGlow} />
      
      {/* Sword trail segments */}
      {trailPositions.map((trail, i) => (
        <mesh
          key={`trail-${i}`}
          position={trail.position}
          geometry={pooledResources.geometries.trailSegment}
          material={pooledResources.materials.trail}
          rotation={[0, 0, (i - 2.5) * 0.3]} // Vary rotation for dynamic look
        />
      ))}
      
      {/* Spark particles along the arc */}
      {particlePositions.map((particle, i) => (
        <mesh
          key={`particle-${i}`}
          position={particle.position}
          geometry={pooledResources.geometries.particle}
          material={pooledResources.materials.particle}
        />
      ))}

      {/* Dynamic lighting */}
      <pointLight 
        color="#8B0000" 
        intensity={8} 
        distance={4} 
        decay={2}
        position={[0, 0, 0]}
      />
      <pointLight 
        color="#A00000" 
        intensity={4} 
        distance={6} 
        decay={2}
        position={[0, 0.5, 0]}
      />
    </group>
  );
}
